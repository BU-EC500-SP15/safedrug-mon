...
object Driver {
  ...
  def readParameter(twitterParam:String) : Option[String] = {
    if(! new File("/root/twitter").exists()){
    ...
    def read(param:String):Option[String]= {
      scala.io.Source.fromFile("/root/twitter").getLines().foreach {
        x =>
          System.out.println("line : " + x)
          if (! x.contains("=")){
            System.err.println("MAJOR failure.  Bad line " +x + " in /root/twitter.")
          }
      ...
      */
      {
        (statuses,sparkConf) =>
          //assumes session.
          statuses.foreach(s => {
            System.out.println("--------------------" + CtakesTermAnalyzer.analyze(s.getText))
          })
          true;
      }
    )
